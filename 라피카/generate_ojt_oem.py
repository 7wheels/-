...
def build():
    ...
    # ── 1. 신청배경(목적) ───────────────────────────────────────────
    s.append(sec('1. 신청배경 (목적)'))
    s.append(sp(1))
    s.append(subsec('1.1 시장·산업 환경'))
    s.extend(bullet([
        ...
        '가격 경쟁이 심화됨에 따라 단순 수주 대응을 넘어 <b>계획·일정·생산성</b>을 정량 관리하는 OEM이 살아남는 구조가 형성되었다.',
    ]))
    s.append(sp(1))
    s.append(subsec('1.2 기업 내부 필요'))
    s.extend(bullet([
        '작년에 품질관리(QC) 과정을 이수하여 검사·불량 저감 측면의 기반은 마련되었으나, <b>생산계획·라인 운영·납기 관리는 여전히 개인 경험에 의존</b>하고 있다.',
        '협력공장(외주) 비중이 늘어나면서 자사·외주 일정 통합 관리, 발주·납기 가시성 확보 필요성이 커졌다.',
        '일일 생산 데이터가 일관된 양식으로 집계되지 않아 경영진 의사결정 자료로 즉시 활용되지 못하는 한계가 있다.',
    ]))
    s.append(sp(1))
    s.append(subsec('1.3 훈련 목적'))
    s.extend(bullet([
        '화장품 OEM 생산관리 직무를 <b>개인 경험 의존에서 표준 양식·정량 KPI 운영</b>으로 전환한다.',
        '사무실 환경에서도 수행 가능한 <b>데이터 기반 생산관리 역량</b>을 사내에 정착시킨다.',
        '작년 QC 과정과 차별화하여 <b>생산성·납기·외주 관리</b> 영역의 직무 역량을 보강한다.',
        '정부 환급 직업능력개발훈련 제도를 활용해 <b>훈련비 부담 없이 사내 인력의 직무 표준화</b>를 달성한다.',
    ]))
    s.append(PageBreak())

    # ── 2. 개발 요구사항 ──────────────────────────────────────────
    s.append(sec('2. 개발 요구사항'))
    s.append(sp(1))
    s.append(subsec('2.1 이해관계자별 요구'))
    s.append(sp(0.5))
    s.append(tbl(
        ['이해관계자', '핵심 요구사항'],
        [
    ...
    # ── 3. 기업 현황 분석 ──────────────────────────────────────────
    s.append(sec('3. 기업 현황 분석'))
    s.append(sp(1))
    s.append(subsec('3.1 일반 현황'))
    s.append(sp(0.5))
    s.append(tbl(
        ['구분', '내용'],
        [
    ...
    s.append(tbl(
        ...
        widths=[22*mm, W - 2*M - 22*mm]))
    s.append(sp(2))
    s.append(subsec('3.3 생산관리 As-is / To-be'))
    s.append(sp(0.5))
    s.append(tbl(
        ['영역', 'As-is (현재)', 'To-be (도달 목표)'],
        [
            ['수주·일정', '부서장 1인의 경험 판단', 'MPS 표준 양식 / 자사·외주 통합 일정 관리'],
            ['외주 관리', '이메일·전화 중심, 통합 대장 부재', '발주·납기 통합 관리대장 운영'],
            ['일일 실적', '종이 양식·개별 엑셀 분산 기록', '표준 일일 생산일보 / 일별 마감 운영'],
            ['KPI 운영', '비주기 산출, 활용 미흡', 'KPI 대시보드 / 일별·주별 추이 시각화'],
            ['개선활동', '이슈 발생 시 사후 대응', '개선 전·후 정량 비교 보고 / 지속 사이클'],
        ],
        widths=[28*mm, (W - 2*M - 28*mm)/2, (W - 2*M - 28*mm)/2]))
    s.append(PageBreak())

    # ── 4. 훈련대상 업무 선정 및 분석 ──────────────────────────────
    s.append(sec('4. 훈련대상 업무 선정 및 분석'))
    s.append(sp(1))
    s.append(subsec('4.1 훈련 대상 직무'))
    s.extend(bullet([
        '<b>직무명:</b> 화장품 OEM 생산관리 / 공정관리',
        '<b>선정 사유:</b> 작년 QC 과정과 차별화 / 사무실 기반 실습 가능 / 공단 요구 정량 KPI 입증 가능 / 사내 표준화 효과 최대',
    ]))
    s.append(sp(1))
    s.append(subsec('4.2 직무 Task 분석'))
    s.append(sp(0.5))
    s.append(tbl(
        ['Task', '현재 수준 (As-is)', '도달 수준 (To-be)', '단원'],
        [
            ['수주 → MPS 수립', '부서장 경험 판단', '표준 시트 1시간 내 (±10% 오차)', '7-1'],
            ['자사·협력공장 일정 조율', '이메일·전화 중심', '발주·납기 관리대장 통합 운영', '7-1'],
            ['표준작업·표준시간', '미운영', '영상 분석으로 ST·라인밸런싱 효율 산출', '7-2'],
            ['일일 생산실적 집계', '분산 기록', '표준 양식 일보 매일 마감', '7-3'],
            ['KPI 모니터링', '비정기', 'Excel/Sheets 대시보드 일별·주별 운영', '7-4'],
            ['개선활동·보고', '사후 대응', '개선 전·후 정량 비교 보고서 작성', '7-5'],
        ],
        widths=[40*mm, 45*mm, 60*mm, 14*mm]))
    s.append(sp(2))
    s.append(subsec('4.3 훈련 대상자 선정 기준'))
    s.extend(bullet([
        '화장품 OEM 또는 제조업 <b>생산관리·공정관리 직무 수행자</b>',
        '<b>실무 경력 1년 이상</b> (단, 부서장이 필요하다고 판단한 신입 포함 가능)',
        '협력공장 관리·외주 발주 업무 담당자 포함 가능',
        '대표이사·임원의 경영 의사결정 보조 인력 포함 가능',
    ]))
    s.append(sp(1))
    s.append(subsec('4.4 사전 진단 (역량 갭)'))
    s.extend(bullet([
    ...
    # ── 5. 기업 훈련환경 분석 ──────────────────────────────────────
    s.append(sec('5. 기업 훈련환경 분석'))
    s.append(sp(1))
    s.append(subsec('5.1 물리적 환경'))
    s.append(sp(0.5))
    s.append(tbl(
        ['구분', '내용'],
        [
    ...
    s.append(tbl(
        ...
        [
            ...
            ['영상 자료', '사전 녹화된 충진·포장 작업 영상 (트레이너 사전 제작, 각 30분)'],
            ['표본 데이터', '5일치 표본 생산 데이터 세트 (라피카 과거 실데이터 가공)'],
            ['양식 템플릿', 'MPS·발주·표준시간·라인밸런싱·일보·KPI 대시보드·비교 보고서 등 7종 사전 제공'],
        ],
        widths=[28*mm, W - 2*M - 28*mm]))
    s.append(sp(1.5))
    s.append(subsec('5.3 인적 환경 / 5.4 운영 가능 시간'))
    s.extend(bullet([
        '<b>OJT 트레이너:</b> 사내 생산관리 부서장·팀장(경력 5년 이상) 중 선정 / 보조 트레이너 1명(KPI 대시보드 1:1 코칭용)',
        '<b>훈련생:</b> 생산관리·공정관리 담당자 (협력공장 관리자 포함 가능)',
        '<b>외부 자문:</b> 히어컴퍼니 기업컨설팅 (과정 설계·평가 도구 제공)',
        '<b>권장 운영:</b> 1주 집중형 (월~금 09:00~18:00)',
        '<b>대안:</b> 분할 운영(예: "월화수 + 다음주 월화" / "월화 + 다음주 수목금")',
    ]))
    s.append(sp(1.5))
    s.append(subsec('5.5 제약 조건 및 대응'))
    s.append(sp(0.5))
    s.append(tbl(
        ['제약 조건', '대응 방안'],
        [
            ['실험·연구장비 부재', 'R&D·처방개발형 단원 제외, 데이터·영상 기반 실습으로 설계'],
            ['라인 가동 중단 최소화', '사무실 회의실 기반 실습, 비훈련 인력으로 라인 운영 유지'],
            ['소규모 인력', '트레이너 1인 + 보조 1인 운영 / 분할 일정 옵션 제공'],
            ['작년 QC 과정과 차별화', '직무 영역·산출물·측정 지표·훈련 환경 전면 차별화 (부록 B)'],
        ],
        widths=[45*mm, W - 2*M - 45*mm]))
    s.append(PageBreak())

    # ── 6. 과정 개요 ────────────────────────────────────────────────
    s.append(sec('6. 과정 개요'))
    s.append(sp(1))
    s.append(subsec('6.1 기본 정보'))
    s.append(sp(0.5))
    s.append(tbl(
        ['항목', '내용'],
        [
    ...
    s.append(subsec('6.2 훈련 목표 (정량 KPI 포함)'))
    s.append(sp(0.5))
    ...
    # ── 2. 단원별 상세 ─────────────────────────────────────────────
    s.append(sec('7. 단원별 상세 (5단원 × 8시간 = 40시간)'))
    s.append(sp(1))
    ...
    s.append(sec('8. 평가'))
    s.append(sp(1))
    s.append(subsec('8.1 평가기준 10개 (이론 4 + 실습 6)'))
    s.append(sp(0.5))
    eval_rows = [
    ...
    # ── 5. 트레이너 ────────────────────────────────────────────────
    s.append(sec('10. OJT 트레이너 요건'))
    s.append(sp(1))
    s.append(subsec('10.1 자격 요건'))
    s.extend(bullet([
    ...
    # ── 6. 일정 ────────────────────────────────────────────────────
    s.append(sec('11. 훈련 일정'))
    s.append(sp(1))
    s.append(subsec('11.1 5일 집중형 일정표 (월~금)'))
    s.append(sp(0.5))
    s.append(tbl(
    ...
    s.append(subsec('11.2 일일 운영 표준 시간표'))
    s.append(sp(0.5))
    s.append(tbl(
        ['시간', '활동', '비고'],
        [
