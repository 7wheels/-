...
SUBTITLE= '5일 × 8시간 집중형 (총 40시간) — 2026년 재직자 직무능력향상훈련'
...
def build():
    ...
    s.append(Table([
        ...
        [Paragraph('<b>운영 방식</b>', ps(KRB, 10, NAVY)),
         Paragraph('5일 × 8시간 집중형 (월~금 1주 완성)',
                   ps(KR, 10, colors.black))],
        [Paragraph('<b>NCS 직무</b>', ps(KRB, 10, NAVY)),
         Paragraph('17 화학 &gt; 01 화학물질·화학제품제조 &gt; 04 정밀화학·화장품제조 &gt; 02 화장품제조',
                   ps(KR, 10, colors.black))],
        [Paragraph('<b>대상</b>', ps(KRB, 10, NAVY)),
         Paragraph('생산관리·공정관리 담당자 5~8명',
                   ps(KR, 10, colors.black))],
    ...
    s.append(tbl(
        ...
        [
            ...
            ['소분류', '04', '정밀화학·화장품제조'],
            ['세분류', '02', '화장품제조'],
            ['능력단위', '1701040201_xxvx', '화장품 생산계획 수립'],
            ['능력단위', '1701040202_xxvx', '화장품 생산공정 관리'],
            ['능력단위', '1701040203_xxvx', '화장품 생산실적 관리'],
            ['능력단위', '1701040204_xxvx', '화장품 충진·포장 관리'],
            ['능력단위요소', '1701040201_xxvx.1', '수주·BOM 분석 및 생산계획 수립하기'],
            ['능력단위요소', '1701040201_xxvx.2', '자사·외주 라인 부하 분배하기'],
            ['능력단위요소', '1701040202_xxvx.1', '배합·충진·포장 표준작업 관리하기'],
            ['능력단위요소', '1701040202_xxvx.2', '라인 밸런싱·표준시간 관리하기'],
            ['능력단위요소', '1701040203_xxvx.1', '일일 생산실적·KPI 집계하기'],
            ['능력단위요소', '1701040203_xxvx.2', '생산성 개선활동 수행하기'],
        ],
        widths=[28*mm, 42*mm, W - 2*M - 70*mm]))
    s.append(sp(1.2))
    s.append(para(
        '※ 본 과정 대분류 17(화학)은 NCS 환급 제외 대분류(01·03·04·07·10·11·12·13)에 미해당. '
        '라피카가 화장품 OEM 제조업체이므로 17 화학 &gt; 화장품제조 직무 매핑이 직무 정합성을 가장 높게 충족함.',
        size=8.5, color=GRAY))
    s.append(para(
        '⚠ 능력단위·요소 코드 끝자리(_xxvx)는 NCS 사이트(www.ncs.go.kr)에서 화장품제조 세분류 최신 개정본의 정확한 버전(예: _22v1, _23v2 등)으로 신청서 제출 전 반드시 재대조하여 기재할 것.',
        size=8.5, color=ORANGE))
    s.append(sp(2))

    s.append(subsec('1.3 훈련 목표 (정량 KPI 포함)'))
    s.append(sp(0.5))
    ...
    s.extend(bullet([
        ...
        '충진·포장 라인의 사이클타임을 스톱워치로 측정하여 <b>공정별 표준시간을 분 단위 소수점 1자리</b>까지 산출하고, 라인 밸런싱 효율을 % 단위로 계산할 수 있다.',
        '일일 생산일보를 매일 17시까지 마감 작성하여 <b>계획 대비 실적률·불량률·시간당 생산량(UPH)</b> 3대 KPI를 산출할 수 있다.',
    ...
    units = [
        {
            ...
            'rows': [
                ...
                ['1-4', '자사 vs 협력공장 부하 분배 실습', '0', '1.5', '자사·협력공장 capacity 부하 분배'],
        ...
        },
        {
            ...
            'title': '배합·충진·포장 공정 표준시간 측정과 라인 밸런싱',
            'time': '8h: 이론 3h + 실습 5h',
            'goals': [
        ...
        },
        {
            ...
            'rows': [
                ...
                ['5-5', '발표 자료 준비 및 발표', '0', '1.5', '슬라이드 5장 + 5분 발표 + Day 5 일보 1장'],
                ['<b>합계</b>', '', '<b>3.5</b>', '<b>4.5</b>', '<b>8.0</b>'],
            ],
            'practice': [
    ...
    eval_rows = [
        ...
        ['5', '실습', '1', '수주서 3건 기반 월간 MPS 시트를 1시간 이내에 작성한다', '산출물(MPS)'],
        ...
        ['8', '실습', '3', '5일치 일일 생산일보를 매일 17시까지 마감 작성하고 실적률·불량률·UPH를 정확히 입력한다', '출석부+산출물'],
    ...
    s.extend(bullet([
        ...
        '내용: S-OJT 운영 매뉴얼·평가 체크리스트 활용법 / 단원별 실습 활동 시연 / 산출물 평가 기준·합격선 / 안전교육·이상상황 대응',
    ]))
    s.append(sp(1))
    s.append(subsec('5.3 1인당 지도 가능 훈련생 수'))
    s.extend(bullet([
        '<b>트레이너 1인당 최대 8명</b> (S-OJT 적정 비율 1:8)',
        '라피카 5~8명 운영 시 트레이너 1명으로 충분',
    ...
    s.append(tbl(
        ...
        [
            ...
            ['Day 2<br/>(화)', '09:00–18:00<br/>(8h)', '단원 2<br/>표준시간·라인밸런싱', '이론 3.0<br/>+ 실습 5.0', '표준시간 측정표<br/>라인밸런싱 분석시트'],
    ...
    s.extend(bullet([
        ...
        '5일치 일일 생산일보를 <b>훈련 5일간 매일 1장씩 훈련시간 내에 작성</b> → 환급 인정 훈련시간 외 자율 작성 우려 완전 해소',
        '사업장 라인 가동 영향 최소화: 훈련생 5명 내외 한정, 비훈련 인력으로 라인 운영 유지',
    ...
    s.append(tbl(
        ...
        widths=[18*mm, W - 2*M - 50*mm, 32*mm]))
    s.append(sp(3))
    ...
    s.append(sp(1.5))
    s.append(para(
        '→ 직무 영역·NCS 코드·산출물·측정 지표가 전면 차별화되어 작년 QC 과정과 중복되지 않음.',
        9.5, KRB, GREEN))
